...
export default function Page2({ page2machine }: Page2Props) {
  const [selectedCategory, setSelectedCategory] = useState<"all" | "servoDriven" | "mechanicalCam">("all")
  ...
  const handleCategoryChange = (category: "all" | "servoDriven" | "mechanicalCam") => {
  ...
  const displayedParagraph = isExpanded ? page2machine.paragraph : truncateText(page2machine.paragraph, wordLimit)
  ...
  const filteredData =
    selectedCategory === "all"
      ...
      ? page2machine.all.flatMap((item) => (Array.isArray(item.servoDriven) ? item.servoDriven : []))
      : page2machine.all.flatMap((item) => (Array.isArray(item.mechanicalCam) ? item.mechanicalCam : []))
  ...
  const handleItemSelection = (itemId: string, itemName: string, itemImage: string) => {
    setSelectedItems((prevSelected) => {
      const newSelected = new Set(prevSelected)
      if (newSelected.has(itemId)) {
        newSelected.delete(itemId)
        setEnquiryItems((prevItems) => prevItems.filter((item) => item.id !== itemId))
      } else {
        newSelected.add(itemId)
        setEnquiryItems((prevItems) => {
          const itemExists = prevItems.some((item) => item.id === itemId)
          if (!itemExists) {
            return [...prevItems, { id: itemId, name: itemName, image: itemImage }]
          }
          return prevItems
        })
      }
  ...
  return (
    <>
      <div className="font-poppins font-regular w-full">
        <div ref={filterRef} className="py-[1rem] lg:px-[2.5rem] px-[0.8rem] z-10 w-full">
              ...
              >
                <div
                  className={`lg:ml-1 ml-[0.1rem] lg:w-[1.4rem] w-[1.1rem] h-[1.1rem] lg:h-[1.4rem] rounded-full flex items-center justify-center lg:text-[0.9rem] text-[0.7rem] font-semibold ${
                    selectedCategory === "servoDriven" ? "text-red-700 bg-white" : "text-white bg-black"
                  }`}
                >
                  S
                </div>
                <button className="lg:ml-[1.4rem] ml-[0.3rem]">{ProductsGrid.servoDriven}</button>
              </div>
              <div
              ...
              >
                <div
                  className={`lg:ml-1 ml-[0.1rem] lg:w-[1.4rem] w-[1.1rem] h-[1.1rem] lg:h-[1.4rem] rounded-full flex items-center justify-center lg:text-[0.9rem] text-[0.6rem] font-semibold ${
                    selectedCategory === "mechanicalCam" ? "text-red-700 bg-white" : "text-white bg-black"
                  }`}
                >
                  M
                </div>
                <button className="lg:ml-[1.4rem] ml-[0.3rem]">{ProductsGrid.mechanicalCam}</button>
              </div>
            </div>
            <div>
            ...
            >
              <div className="absolute top-6 right-4 flex space-x-2">
                <div className="w-6 h-6 p-[0.2rem] bg-[#f5f5f5] border-solid border-[0.1rem] border-[#f5f5f5] hover:border-red-700 rounded-full flex items-center justify-center relative group">
                  ...
                  <div className="hidden group-hover:flex absolute bottom-7 right-0 bg-white border border-gray-300 rounded-md shadow-md p-3 h-max w-max z-20">
                    <p className="text-sm text-black">{item.imageInformation}</p>
                  </div>
                </div>
                <div className="w-6 h-6 bg-[#f5f5f5] rounded-full flex items-center justify-center font-medium cursor-pointer relative group hover:text-red-700">
                  {item.s}
                  <div className="hidden group-hover:flex absolute bottom-7 right-0 bg-white border border-gray-300 rounded-md shadow-md p-3 h-max w-max z-20">
                    <p className="text-sm text-black font-normal">{item.sInformation}</p>
                  </div>
                </div>
              ...
              <div className="flex justify-center items-center overflow-hidden">
                <div className="mt-[1rem] lg:p-6 p-4 lg:h-[16rem] flex justify-center items-center">
                  <Image src={item.img} alt="" priority width={400} height={400} />
                </div>
              </div>
                ...
                {hoveredCardIndex !== idx ? (
                  <div className="flex w-full items-center justify-center px-2">
                    <div className="text-center w-[33%]">
                      <p className="font-semibold lg:text-md text-[0.9rem]">{item.range}</p>
                      <p className="text-gray-600 lg:text-md text-[0.8rem]">{item.rangeTitle}</p>
                    </div>
                    <div className="h-8 w-px bg-[#9c9c9c]"></div>
                    <div className="text-center  w-[33%]">
                      <p className="font-semibold lg:text-md text-[0.9rem]">{item.punch}</p>
                      <p className="text-gray-600 lg:text-md text-[0.8rem]">{item.punchTitle}</p>
                    </div>
                    <div className="h-8 w-px bg-[#9c9c9c]"></div>
                    <div className="text-center  w-[33%]">
                      <p className="font-semibold lg:text-md text-[0.9rem]">{item.weight}</p>
                      <p className="lg:text-md text-[0.8rem] text-gray-600">{item.weightTitle}</p>
                    </div>
                  </div>
                ) : (
                  <button
              ...
              <div className="my-4 flex items-center justify-center">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={() => handleItemSelection(`${idx}`, item.h1, item.img)}
                  />
                  <Label htmlFor={`addToEnquiry-${idx}`} className="text-sm whitespace-nowrap">
